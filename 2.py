...
file = opx.load_workbook('book.xlsx')
...
class Book:
    ...
    def return_book(self):
        self.borrowed = False
        logger.info(f'{self.name} has been returned')
...
class Shelf:
    ...
    def show_catalogue(self):
        if len(self.shelf_arr) > 0:
            ...
            for book in self.shelf_arr:
                table_dat.append([str(book.name), str(book.ISBN), book.author])
    ...
    def add_book(self, user: User):  # LibUser only use decs
        try:
    ...
    def remove_book(self, user: User, book_del):  # LibUser only use decs
        try:
            if user.role == 'Librarian':
